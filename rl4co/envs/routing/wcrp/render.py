...
def render(td, actions=None, ax=None):
    ...
    # Plot depot and bins with prize
    ax.scatter(
        depot[0],
        depot[1],
        marker="s",
    ...
    ax.scatter(
        ...
        alpha=0.6,
        edgecolors="black",
    )  # Plot all bins with size and color indicating the prize

    # Gather locs in order of action if available
    if actions is None:
        log.warning("No action in TensorDict, rendering unsorted locs")
    else:
        # Reorder the bins and their corresponding prizes based on actions
        tour = bins[actions - 1]  # subtract 1 to match Python's 0-indexing
